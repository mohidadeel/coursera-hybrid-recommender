...
def load_and_train_system():
    ...
    # 5. ADVANCED NLP: SBERT + FAISS Vector Indexing
    sbert_model = SentenceTransformer('all-MiniLM-L6-v2')
    course_corpus = unique_courses['name'].fillna('').tolist()
    
    # Encode courses to dense vectors (384 dimensions)
    course_embeddings = sbert_model.encode(course_corpus, convert_to_numpy=True)
    
    # Normalize vectors for Cosine Similarity inside FAISS
    faiss.normalize_L2(course_embeddings)
    dimension = course_embeddings.shape[1]
    
    # Build Inner Product FAISS Index (equivalent to Cosine Similarity for normalized vectors)
...
st.markdown("This app integrates SBERT semantic search, FAISS vector indexing, and a Sentiment-Aware Hybrid Recommender to find courses based on true pedagogical value.")
...
with tab1:
    if search_query.strip() == "":
        ...
        # 1. DYNAMIC SEMANTIC SEARCH VIA SBERT + FAISS
        query_vector = sbert_model.encode([search_query], convert_to_numpy=True)
        faiss.normalize_L2(query_vector)
        
        # Search the entire index to get similarities for all courses to merge with SVD
        k_results = len(unique_courses) 
        similarities, indices = faiss_index.search(query_vector, k_results)
        
        # Map FAISS results back to the dataframe
        candidates = unique_courses.copy()
        
        # Create a temporary array to store match scores in the correct original order
        match_scores = np.zeros(len(candidates))
        for rank, original_idx in enumerate(indices[0]):
            match_scores[original_idx] = similarities[0][rank]
            
        candidates['content_match_score'] = match_scores
        ...
        if candidates.empty or candidates['content_match_score'].max() == 0:
            ...
            st.markdown(f"### 🎯 Curated Best-Fit Learning Paths for: {selected_user}")
            ...
            display_df.columns = ['Course Name', 'Offered By', 'Difficulty', 'Credential Type', 'Total Registrations', 'Predicted User Quality Rank']
